...
if __name__ == '__main__':
    ...
    # Input parameters
    index = 2
    dload = './Cricket_model_dir/' + str(index)  # download directory
    ...
    X_train, X_val, y_train, y_val = open_Cricketdata_6d('Cricket_6d', ratio_train=0.7, dataset="CricketDimension")
    ...
    config.dload = dload
    vrae = main(config)
    # vrae.fit(train_dataset, save=True, mode="cricket_6d")
    # vrae.save('vrae.pth')
    vrae.load('./Cricket_model_dir/{}/vrae.pth'.format(str(Model_Num)))
    # # loss = vrae.compute_loss(test_dataset)

    # # # # Transform the input timeseries to encoded latent vectors
    # z_run = vrae.transform(test_dataset,mode="cricket_6d")
    ...
    X_decoded = vrae.reconstruct(test_dataset, mode="cricket_6d")
    # print("X_decoded", X_decoded.shape)
    for i in range(10):
        # Plotting test_dataset
        test_data_sample = np.squeeze(test_dataset.tensors[0].numpy()[i, :])
        print("test_data_sample", test_data_sample.shape)
        for j in range(6):
            test_data_sample_time_steps = np.arange(0, test_data_sample.shape[0])
            plt.plot(test_data_sample_time_steps, test_data_sample[:,j])
            X_decoded_sample = np.squeeze(np.squeeze(X_decoded)[:, j])
            X_decoded_sample_time_steps = np.arange(0, X_decoded_sample.shape[0])
            plt.plot(X_decoded_sample_time_steps, X_decoded_sample[:,j])
            plt.title("Dimension {}".format(j+1))
            plt.savefig("./Cricket_model_dir/{}/plots/{}_{}compare.png".format(str(Model_Num), str(i),str(j)))
