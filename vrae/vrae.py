...
class Lambda(nn.Module):
    ...
    def forward(self, cell_output):
        ...
        if self.training:
            ...
            return eps.mul(std).add_(self.latent_mean)  # =latent
        else:
...
class VRAE(BaseEstimator, nn.Module):
    ...
    def __init__(self, sequence_length, number_of_features, hidden_size=90, hidden_layer_depth=2, latent_length=20,
                 ...
                 cuda=False, print_every=100, clip=True, max_grad_norm=5, dload='.', cnn_mode=False):
    ...
    def _train(self, train_loader, cnn_mode=False, rgb_mode=False, mode=None):
        """
        ...
        for t, X in enumerate(train_loader):
            if cnn_mode == True:
                # im, label = X
                # X = X.reshape(self.batch_size, self.sequence_length, self.number_of_features)

                # X = X.squeeze()
                if rgb_mode:
                    X = X[0]  # image # torch.Size([16, 3, 32, 32])
                    X = X.permute(0, 3, 2, 1)  # torch.Size([16, 32, 32, 3])
                    # todo best practice
                    # X = X.reshape(self.batch_size, 32, 32 * 3)  # from torch.Size([16, 32, 32, 3]) to torch.Size([16, 32, 96])
                    X = X.reshape(self.batch_size, 32 * 32, 3)  # from torch.Size([16, 32, 32, 3]) to torch.Size([16, 32, 96])

                    X = X.permute(1, 0, 2)  # from torch.Size([16, 32, 96]) to torch.Size([32, 16, 96]) batch size in the middle
                else:
                    X = X[0]  # image
                    X = X.squeeze()  # torch.Size([16, 96, 1])
                    # print(X.size())
                    X = X.permute(1, 0, 2)  # 將最後一維放到第一維，變成（28, batch，28）
                    # print("X.permute(2, 0, 1)", X.size())
                    # X = X.permute(2, 0, 1)
            elif mode == "cricket_6d":
                X = X[0]
                X = X.squeeze()
                # X = X.reshape(self.batch_size, -1, 6)
                # print("before X = X.permute(1, 0, 2)", X.size()) # torch.Size([16, 6, 1197, 1])
                X = X.permute(1, 0, 2)
                # print("X = X.permute(1, 0, 2)", X.size())# torch.Size([96, 16, 1])
            else:
                # Index first element of array to return tensor
                X = X[0]
                # print("before X = X.permute(1, 0, 2)", X.size())
                # required to swap axes, since dataloader gives output in (batch_size x seq_len(colume 28/32) x num_of_features(27/32*3))
                X = X.permute(1, 0, 2)
                # print("X = X.permute(1, 0, 2)", X.size())# torch.Size([96, 16, 1])
    ...
    def fit(self, dataset, save=False, mode=None):
        """
        ...
        for i in tqdm(range(self.n_epochs)):
            ...
            self._train(train_loader, mode=mode)

        self.is_fitted = True
        if save:
            self.save('model.pth')

    def fit_cnn(self, train_loader, save=False, cnn_mode=False, rgb_mode=False, mode=None):
        self.cnn_mode = True
        for i in tqdm(range(self.n_epochs)):
            print('Epoch: %s' % i)

            self._train(train_loader, cnn_mode, rgb_mode)
    ...
    def reconstruct(self, dataset, save=False, cnn_mode=False, rgb_mode=False,mode=None):
        """
                ...
                for t, x in enumerate(test_loader):
                    if cnn_mode:
                        if rgb_mode:
                            ...
                            print(" x[0]  ", x.size())
                            # TODO best practice
                            # x = x.view(-1, 32, 32 * 3)  # torch.Size([16, 32, 96])
                            x = x.view(-1, 32 * 32, 3)
                            # print(x.size())
                            print(" x.view(-1, 32, 32 * 3) ", x.size())
                            x = x.permute(1, 0, 2)  # torch.Size([32, 16, 96])
                            print(" x.permute(1, 0, 2) ", x.size())
                            # print(x.size())
                        else:
                            x = x[0]
                            x = x.squeeze()
                            x = x.permute(1, 0, 2)  # 將最後一維放到第一維，變成（batch，28,28）
                        # print(x.size())
                        x_decoded_each = self._batch_reconstruct(x)
                        print(" x_decoded_each ", x_decoded_each.size)
                        x_decoded.append(x_decoded_each)
                        return x_decoded_each
    ...
    def reconstruct_cnn(self, test_loader, save=False):
        """
        Given input dataset, creates dataloader, runs dataloader on `_batch_reconstruct`
        Prerequisite is that model has to be fit
    ...
    def transform(self, dataset, save=False, cnn_mode=False, rgb_mode=False, mode=None):
        """
