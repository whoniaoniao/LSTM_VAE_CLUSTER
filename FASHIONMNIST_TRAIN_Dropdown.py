...
def update_output(fig_dropdown):
    return name_to_figure(fig_dropdown)


def name_to_figure(fig_dropdown):
    figure = go.Figure()
    if fig_dropdown == 'KEMAN':
        for i in range(10):
        ...
        for i in range(10):
            figure.add_trace(go.Scatter(x=list(z_run_sep[colors == i, 0]),
                                        ...
                                        name=str(i)))
    return dcc.Graph(figure=figure, id='fig_plot')
